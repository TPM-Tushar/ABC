...
$(document).ready(function () {
    ...
    $("#btnDownloadDROScript").click(function () {
        window.location.href = "/SROScriptManager/SROScriptManager/DownLoadDROScriptFile?ScriptID=" + $("#hdnScriptID").val();
    });

    $("#btnUpdateDROScript").click(function () {
        if ($('#rdnEnable').is(':checked'))
            $("#hdnIsActive").val('True');
        else
            $("#hdnIsActive").val('False');
        UpdateInDROScriptManger();
    });
...
function UpdateInDROScriptManger() {

    //console.log("in UpdateInScriptManger");

    //alert("file :: " + $("input[type='file']").val());
    ...
    //alert("filesArray :: " + filesArray);

    formData.append("filesArray", filesArray);
    var form = $("#DROScriptManagerUpdateForm");
    form.removeData('validator');
    form.removeData('unobtrusiveValidation');
    //$.validator.unobtrusive.parse(form);

    if ($("#DROScriptManagerUpdateForm").valid()) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    $("#hdnDROSQLScriptContent").val(data.FileDataStr);
                    BlockUI();
                    $.ajax({
                        type: "POST",
                        url: "/SROScriptManager/SROScriptManager/UpdateDROScriptManager",
                        data: $("#DROScriptManagerUpdateForm").serialize(),
                        headers: header,
                        success: function (data) {
                            if (data.success) {
                                bootbox.alert({
