...
$(document).ready(function () {
    ...
    //added by vijay 09-03-2023


    const regex = /^[\u0C80-\u0CFFa-zA-Z0-9_\/\\\-()#&*+, ]*$/;
    const inputField = document.getElementById('txtCurrentSurveyNoChar');
    const inputField_hissa = document.getElementById('txtCurrentHissaNo');
    const invalidChar_surnoc = document.getElementById('invalid-char2');
    const invalidChar_Hissa = document.getElementById('invalid-hissa');
    const errorMessage = document.getElementById('txtCurrentSurveyNoChar_err');
    const errorMessage_hissa = document.getElementById('txtCurrentHissaNo_err');
    ...
    inputField.addEventListener('input', () => {
        ...
        if (!regex.test(input)) {
            ...
            if (invalidIndex >= 0) {
                invalidChar_surnoc.textContent = `Invalid character: ${input.charAt(invalidIndex)}`;
                invalidChar_surnoc.style.display = 'block';
            }
        } else {
            errorMessage.style.display = 'none';
            invalidChar_surnoc.style.display = 'none';
        }
    });

    //added by vijay on 09-03-2023
    inputField_hissa.addEventListener('input', () => {
        const input = inputField_hissa.value;
        if (!regex.test(input)) {
            errorMessage_hissa.style.display = 'block';
            const invalidIndex = input.search(/[^[\u0C80-\u0CFFa-zA-Z0-9_\/\\\-()%#&*+, ]/);
            if (invalidIndex >= 0) {
                invalidChar_Hissa.textContent = `Invalid character: ${input.charAt(invalidIndex)}`;
                invalidChar_Hissa.style.display = 'block';
            }
        } else {
            errorMessage_hissa.style.display = 'none';
            invalidChar_Hissa.style.display = 'none';
        }
    });
